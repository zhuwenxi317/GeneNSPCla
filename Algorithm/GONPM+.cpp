...
int threshold;
char S[N];
// 自定义每层（从第2层开始）支持度衰减因子列表，直接在此修改：
// 例如 {0.9, 0.85, 0.8} 表示：第2层*0.9，第3层再*0.85，第4层再*0.8；更高层沿用0.8。
...
int calculate_dynamic_threshold(int level, int base_threshold) {
	...
	int idx = level - 2; // 第2层对应索引0
	double f;

	if (!level_decay_factors.empty()) {
		if (idx < (int)level_decay_factors.size()) {
			f = level_decay_factors[idx];  // 对应层的因子
		}
		else {
			f = level_decay_factors.back(); // 超出范围则使用最后一个固定值
		}
	}
	else {
		f = 0.8;
...
void inneg(string s)
{
	...
	for (int j = 0; j < setlen; j++)
	{
		...
		// 使用按层级的动态阈值（inneg 关联初始层，按 level=1 处理）
...
// 新增：按层级计算动态阈值（第2层开始按用户给出的因子逐层累计衰减）
...
int main() {
	...
	// 在代码中直接设置每层衰减因子（从第2层开始）。需要变更时只改这行：
	level_decay_factors = { 0.8, 0.75}; // 示例：L2*0.80, L3及以后*0.70（按需修改）

	// 显示层级阈值策略预览（前5层）
	cout << "\n===== 层级阈值策略（预览前5层） =====" << endl;
		for (int lv = 1; lv <= 5; ++lv) {
			int thr = calculate_dynamic_threshold(lv, threshold);
			cout << "层级 " << lv << ": 阈值 = " << thr << endl;
		}
	cout << "========================\n" << endl;
	...
	if (outFile.is_open()) {
		...
		outFile.close();
		cout << "纯模式数据已保存至：D:\\CM-SPAM_data\\example_ans.txt" << endl;
	}
	else {
	...
	cout << "\n===== 挖掘到的频繁序列（分层显示） =====" << endl;
	for (int i = 1; i < f_level; ++i) { // 遍历层级≥1的数据
		if (!freArr[i].empty()) {
			cout << "层级" << i << "：" << endl; // 添加层级标识
			for (const auto& pattern : freArr[i]) {
				cout << "  " << pattern << endl; // 模式缩进显示
			}
			cout << endl; // 层级间空行分隔
		}
	}
